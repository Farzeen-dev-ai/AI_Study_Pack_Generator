...
def _call_ai(model, prompt_text, stage_name, retries=2):
    """Call Gemini with simple retry and validation handling."""
    last_error = None
    ...
    raise WorkflowError(
        f"{stage_name} failed after {retries + 1} attempts: {last_error}"
    )


def planning_stage(model, user_input):
    raw = _call_ai(
        model,
        build_planning_prompt(user_input),
        "Planning stage",
    )
    ...
    if missing:
        raise WorkflowError(
            f"Planning stage is missing required fields: {', '.join(missing)}"
        )

    return plan


def content_generation_stage(model, user_input, plan):
    raw = _call_ai(
        model,
        build_content_prompt(user_input, plan),
...
def assessment_stage(model, user_input, plan, content):
    raw = _call_ai(
        model,
        build_assessment_prompt(user_input, plan, content),
        "Assessment stage",
    )
...
def review_stage(model, user_input, plan, content, assessment):
    raw = _call_ai(
        model,
        build_review_prompt(user_input, plan, content, assessment),
        "Review stage",
    )
    ...
    if missing:
        raise WorkflowError(
            f"Review stage is missing required fields: {', '.join(missing)}"
        )

    return review


def refinement_stage(model, user_input, plan, content, assessment, review):
    raw = _call_ai(
        model,
        build_refinement_prompt(
            user_input, plan, content, assessment, review
        ),
        "Refinement stage",
    )
...
def generate_study_pack(
    ...
    Context flow:
    user input -> plan -> content -> assessment -> review -> refinement
    """
    ...
    plan = planning_stage(model, user_input)
    update("Planning", "complete")

    update("Content Generation", "running")
    content = content_generation_stage(model, user_input, plan)
    update("Content Generation", "complete")

    update("Assessment", "running")
    assessment = assessment_stage(model, user_input, plan, content)
    update("Assessment", "complete")

    update("Review", "running")
    review = review_stage(model, user_input, plan, content, assessment)
